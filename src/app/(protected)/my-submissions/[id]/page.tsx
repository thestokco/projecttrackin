  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <Link href="/my-submissions" className="flex items-center gap-2 text-sm text-muted hover:text-foreground">
      ...
      <div className="bg-card rounded-xl border border-border p-6 space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <InfoField
            ...
            label="Cable Return"
            value={
              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${submission.cable_return ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>
        ...
        {submission.remark && (
          <div>
            <label className="block text-sm font-medium text-muted mb-2">Remark</label>
            <div className="bg-gray-50 rounded-lg p-4 text-sm whitespace-pre-wrap">{submission.remark}</div>
          </div>
        )}

        {submission.photos && submission.photos.length > 0 && (
          <div>
            <label className="block text-sm font-medium text-muted mb-3">Photos ({submission.photos.length})</label>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {submission.photos.map((url, i) => (
                <div key={i} className="relative aspect-square rounded-lg overflow-hidden border border-border group">
                  <img src={url} alt={`Photo ${i + 1}`} className="w-full h-full object-cover" />
                  <a href={url} download target="_blank" rel="noopener noreferrer" className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100">
...
function InfoField({ icon, label, value }: { icon: React.ReactNode; label: string; value: React.ReactNode }) {
  return (
    <div>
      <label className="flex items-center gap-1.5 text-sm font-medium text-muted mb-1">{icon}{label}</label>
